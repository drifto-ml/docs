...
```sql
CREATE MODEL my_model WITH (target=y, max_models=10) AS SELECT \* FROM mydata;
```

```sql
CREATE MODEL my_other_model WITH (max_models=1, algo='linear') AS SELECT * FROM mydata;
```
